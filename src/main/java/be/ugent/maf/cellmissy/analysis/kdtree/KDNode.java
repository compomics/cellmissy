...
public class KDNode<T> implements Serializable {
    ...
    protected HPoint k;
    T v; // uses generic type
    protected KDNode<T> left, right;
    protected boolean deleted;

    // edit the node (as of Gonnet & Baeza-Yates); uses the correspondent editor
    ...
    // Method srch (as of Gonnet & Baeza-Yates)
    ...
    // Method rsearch (as of Gonnet & Baeza-Yates)
    ...
    protected static <T> void nnbr(KDNode<T> kd, HPoint target, HRect hr, double max_dist_sqd, int lev, int K, NearestNeighborList<KDNode<T>> nnl,
              Checker<T> checker, long timeout) {
    ...
    private KDNode(HPoint key, T val) {
        ...
        left = null; // left
        right = null; // right
        deleted = false; // is deleted? 
    }

    // Utility to string for debugging mainly
