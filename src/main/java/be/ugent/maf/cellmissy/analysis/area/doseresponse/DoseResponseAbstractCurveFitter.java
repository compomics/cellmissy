...
public abstract class DoseResponseAbstractCurveFitter extends AbstractCurveFitter {
    ...
    */
   public double[] fit(Collection<WeightedObservedPoint> points, SigmoidFittingResultsHolder resultsHolder) {
       // Perform the fit.
       return getOptimizer().optimize(getProblem(points), resultsHolder).getPoint().toArray();
   }
   /**
